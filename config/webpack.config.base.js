...
const getFileNameList = (path,fileList) => {
    ...
    dirList.forEach(function (item) {
        ...
        if (stat.isDirectory()) {
        //递归读取文件
            getFileNameList(path + item + "/",fileList)
        } else {
            if (item.indexOf('pug') > -1) {
...
const htmlDirsss = (path)=>{
        let fileList = []
        getFileNameList(path,fileList);
        return fileList
}

var htmlDirs = htmlDirsss(config.HTML_PATH)
...
htmlDirs.forEach((page) => {
    let htmlConfig = {
        filename: `${page}.html`,
        template: path.join(`${config.HTML_PATH}/${page}`, `./${page}.pug`) // 模板文件
    };
    ...
    module: {
        rules: [{
            ...
            use: [{ // 图片文件小于8k时编译成dataUrl直接嵌入页面，超过8k回退使用file-loader
                ...
                options: {
                    limit: 8192, // 8k
                    name: 'imgs/[name].[ext]', // 回退使用file-loader时的名称
